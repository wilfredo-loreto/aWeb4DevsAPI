...
function getTech(req, res) {
  ...
  Tech.findOne({ title: techTitle }, (err, tech) => {
    ...
    if (!tech)
      return res.status(404).send({ message: `the tech doesn't exists` });
...
function getTechs(req, res) {
  ...
  Tech.find(
    { type: techType },
    { title: 1, summary: 1, tags: 1, logo: 1, parent: 1, _id: 0 },
    (err, techs) => {
      if (err) return res.status(500).send({ message: `error nr: ${err}` });
...
function searchTechs(req, res) {
  ...
  Tech.find(
    { tags: { $in: [keyWord[0], keyWord[1], keyWord[2], keyWord[3]] } },
    { title: 1, type: 1, summary: 1, tags: 1, _id: 0 },
    (err, techs) => {
      if (err) return res.status(500).send({ message: `error nr: ${err}` });
...
function deleteTech(req, res) {
  ...
  Tech.findOne({ title: techTitle }, (err, tech) => {
    tech.remove((err) => {
      if (err)
        return res.status(500).send({ message: `Error deleting tech: ${err}` });
